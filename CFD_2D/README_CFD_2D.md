...
The reproducible aerodynamic preprocessing engine is XFOIL. Its commands,
convergence evidence and generated profile metadata are stored with the case.
The standard mode uses all converged alpha rows. The optimized mode runs the same
...
The independent **Validación** page also evaluates the accepted LS(1)-0417
polar against the Experimental, CFD Cobalt and CFD Kestrel references. It
exports point differences, the paper-normalized RMS `err`, and signed maximum
error `err2`; removing or adding a published point regenerates every product.
Its solver configuration is stored under
`validation_studies/ls1_0417_closed_polar_M0p15_Re1p9e6/configurations`.

For open-airfoil development, **Malla > Estrategia experimental desde cero**
stores independent revisions under
`experimental_meshes/open_reference_from_scratch`. Every size shown in that
editor is relative to chord. A generated revision can be opened and reviewed,
but approval remains explicit and quality warnings remain visible.

The closed experimental editor can use automatic four-arc Bump matching. The
TE cap and curvature-detected LE arc use `C>1`, while the upper and lower wall
arcs use `0<C<1`; the solver finds a common physical end-cell spacing and never
changes the requested divisions behind the user's back. The UI reports the
resulting coefficients, interface ratios, largest tangential cell and local
growth. The equivalent open-airfoil option is deliberately opt-in because the
accepted open baseline still uses its audited legacy wall/inlet controls.

Both experimental meshers also expose an optional Gmsh `Extend` law. In Gmsh
4.15 this law is controlled by `DistMax`, `SizeMax` and `Power`; there is no
`Ratio` option. The source curves are the real variable transfinite Bump curves
that determine BL-column widths. Gmsh evaluates background fields before the
generated outer BL front exists, so that generated front cannot be referenced
directly. `Restrict` confines the external and cavity laws to their respective
surfaces. With either switch off, the original field stack is reproduced.

The open and closed editors now expose one authoritative
`Discretización tangencial de pared` section. Automatic matching solves four
coefficients; disabling it applies the displayed four manual coefficients and
divisions verbatim. The last automatic solution is retained as the manual
starting point. TE resolution is the `Divisiones TE` value in that section.
Historical `te_cap_nodes`, tangential min/max, global wall-node and legacy Bump
keys remain readable by the backend for old revisions, but are intentionally no
longer shown because they do not govern the modern four-segment path.

The reproducible Extend trials are generated by
`ramair_2d_reference_extend_study.py`. On the closed reference, `DistMax=20c`
and `Power=0.5` passes `checkMesh`, but increases maximum non-orthogonality from
31.61 to 41.01 degrees and is therefore retained without replacing the base.
On the open reference, the corresponding external-only law passes; activating
the cavity law reduced the minimum face interpolation weight below 0.05 in all
tested power/distance combinations. Internal Extend consequently remains an
explicit experiment rather than a default or approved revision.

Gmsh cannot attach a background field directly to a BoundaryLayer front that
does not exist until 2-D generation. The external proxy is therefore the exact
transfinite wall/inlet curve discretization that creates the same BL columns;
`SizeMax` is reached at `DistMax` and remains active to the farfield. In the
cavity, the source is the virtual inlet plus shared internal wall curves. This
has matching tangential indexing but is not literally the generated first
prism-front entity, so every candidate must pass native `checkMesh` before use.

`ramair_2d_experimental_control_study.py` provides a bounded reproducible study
of algorithms 5/6, smoothing and the three post-generation optimizers. It writes
`controlled_option_study/comparison.json`. Optimization is applied to a newly
reproduced 2-D revision before extrusion, never to the extruded OpenFOAM mesh;
the immutable base revision is retained and Q is advisory until user approval.

Optional `Laplace2D` optimization is disabled by default. A controlled closed
comparison produced the same limiting quality values with and without it, so
the project treats tangential distribution and geometric segmentation as the
primary quality controls.

The retained manual baseline is
`beta75_continuous_v14_v12_geometry_smooth_farfield_try_refining`. Its inlet guide uses the
uncut source profile over the central opening plus short C1 bridges because the
independently preprocessed cut lips do not lie exactly on that source curve.
The external Delaunay size follows a continuous distance law
`h(d)=min(h_far,h_interface+g*d)` with `g=0.12`; Gmsh has no direct
`nCellsBetweenLevels` equivalent. A complete distribution study is deliberately
manual from the Quality panel. Exact acceptance and extrema always come from
OpenFOAM `checkMesh`; reconstructed MSH2 tables are localization and population
diagnostics only.

The current balanced successor is `beta75_calculated_v20_balanced`. It was
created as a separate revision and does not overwrite the manual baseline.
The limiting low-weight pair was identified as an internal triangular prism
against the first boundary-layer hexahedron at the inlet. Reducing the internal
inlet size factor to 0.30 improved minimum interpolation weight from 0.05112 to
0.07668 and minimum volume ratio from 0.07184 to 0.11073. Reducing the first
external triangle to `3.5e-5 c` independently improved non-orthogonality and
skewness. The combined mesh has 200,001 cells and `checkMesh=OK`.

In Beta mode the UI no longer accepts a manual coefficient. It solves Gmsh's
Beta law from y1, `NbLayers` and `delta99*safety_factor`, then measures the
generated stack. In geometric mode the growth rate is editable and the minimum
integer layer count is derived automatically. The full distribution study is
an explicit Quality-panel action and writes both PNG tables and ParaView VTK
hotspots.

OpenFOAM finite-volume fields are cell-centred. Consequently the y+-derived
wall distance is the distance to the first cell centre and the geometric Gmsh
height is twice that value. For the mandatory 1 m, M=0.15, Re=1.9e6,
rho=0.66606662 kg/m3, mu=1.7894e-5 Pa s and y+=1 check, the pair is
`12.8515e-6 m` centre distance and `25.7031e-6 m` full first-cell height.
With 75 layers and a 1.20 thickness factor, the solved coefficient is
`Beta=1.01573622` and the target stack is `0.02463952 m`. `forceCoeffs`
integrates pressure-normal and viscous-shear
forces, including their moments, over all physical inner and outer airfoil wall
patches.

Case generation reads the selected mesh quality report and applies the same
automatic controls to SIMPLE and PIMPLE: 0 non-orthogonal correctors below 50
degrees, 1 from 50 to below 70, and 2 from 70 degrees; the Laplacian is
`corrected` below 70 and `limited 0.5` from 70 degrees. The values and report
path are persisted in the generated case configuration.
