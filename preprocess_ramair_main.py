...
ENABLE_RIB_INC_TRANSLATION = False
...
WRITE_CHORD_DISTRIBUTION_PLOT = True
...
def write_params_csv(cfg: Config, out_dir: Path, rib_df: pd.DataFrame) -> None:
    ...
    try:
        input_profile_display = cfg.input_csv.resolve().relative_to(PROJECT_ROOT.resolve()).as_posix()
    except (OSError, ValueError):
        input_profile_display = str(cfg.input_csv)
    rows = [
        ["parameter", "value", "unit", "description"],
        ["input_profile_path", input_profile_display, "file", "Source profile path used by the preprocessor."],
...
def _apply_default_case_config(cfg: Config, args: argparse.Namespace) -> Config:
    """Apply the public project JSON to the real CATIA/preprocessor settings.

    The UI edits this file, so every exposed key must affect either ``Config`` or
    an explicitly named 2D/export module switch. Unknown keys remain harmless,
    while type coercion follows the existing setting being replaced.
    """
    ...
    if not isinstance(data, dict):
        raise TypeError(f"Project configuration must contain a JSON object: {config_path}")
    ...
    def resolve_profile_path(raw: str | Path) -> Path:
        """Resolve a profile after the project or portable package is relocated."""
        raw_text = str(raw).strip()
        portable_name = PureWindowsPath(raw_text).name if "\\" in raw_text else Path(raw_text).name
        configured = Path(raw_text)
        direct = configured if configured.is_absolute() else config_root / configured
        configured_profiles = Path(str((data.get("project_paths") or {}).get("profiles_dir", "profiles")))
        profiles_root = (
            configured_profiles
            if configured_profiles.is_absolute()
            else config_root / configured_profiles
        )
        candidates = [
            direct,
            profiles_root / portable_name,
            config_root / "profiles" / portable_name,
            config_root / "Airfoil Profiles" / portable_name,
            PROJECT_ROOT / "profiles" / portable_name,
            PROJECT_ROOT / "Airfoil Profiles" / portable_name,
        ]
        ...
        raise FileNotFoundError(
            f"Configured profile '{raw_text}' was not found. Portable search locations: {searched}"
        )

    def apply_fields(section_name: str, mapping: dict[str, str]) -> None:
        section = data.get(section_name, {})
        if not isinstance(section, dict):
            raise TypeError(f"{section_name} must be a JSON object in {config_path}")
        for public_key, config_key in mapping.items():
            if public_key not in section or config_key not in values:
                continue
            raw = section[public_key]
            current = values[config_key]
            if isinstance(current, bool):
                values[config_key] = bool(raw)
            elif isinstance(current, int) and not isinstance(current, bool):
                values[config_key] = int(raw)
            elif isinstance(current, float):
                values[config_key] = float(raw)
            elif isinstance(current, Path):
                values[config_key] = resolve_config_path(raw)
            elif isinstance(current, tuple):
                values[config_key] = tuple(dict(item) if isinstance(item, dict) else item for item in raw)
            else:
                values[config_key] = str(raw)
    ...
    if profile_inputs.get("main_profile"):
        values["input_csv"] = resolve_profile_path(profile_inputs["main_profile"])
    ...
    apply_fields("canopy_geometry", {
        "chord_mm": "chord_mm",
        "span_mm": "span_total_mm",
        "cells": "cells",
        "anhedral_deg": "arc_anhedral_deg",
        "enable_variable_chord": "enable_variable_chord",
        "chord_mode": "chord_distribution_mode",
        "tip_chord_factor": "tip_chord_factor",
        "chord_anchor": "chord_anchor_mode",
        "chord_anchor_fraction": "chord_anchor_fraction",
        "quasi_elliptic_exponent": "quasi_elliptic_exponent",
        "enable_variable_cell_span": "enable_variable_cell_span",
        "cell_span_mode": "cell_span_distribution_mode",
        "tip_cell_width_factor": "tip_cell_width_factor",
        "enable_span_shrinkage": "enable_span_shrinkage",
        "span_shrinkage_fraction": "span_shrinkage_fraction",
        "anhedral_arc_mode": "anhedral_arc_mode",
    ...
    if "chord_mode" in canopy and "enable_variable_chord" not in canopy:
        values["enable_variable_chord"] = str(canopy["chord_mode"]) != "rectangular"
    if "cell_span_mode" in canopy and "enable_variable_cell_span" not in canopy:
        values["enable_variable_cell_span"] = str(canopy["cell_span_mode"]) != "uniform"

    apply_fields("rib_and_cell_geometry", {
        key: key for key in [
            "enable_rib_inc_translation",
            "loaded_rib_incidence_deg",
            "nonloaded_rib_incidence_deg",
            "loaded_rib_vertical_offset_mm",
            "nonloaded_rib_vertical_offset_mm",
            "loaded_rib_chordwise_offset_mm",
            "nonloaded_rib_chordwise_offset_mm",
            "tip_extra_incidence_deg",
            "tip_extra_vertical_offset_mm",
            "enable_cell_ballooning",
            "max_thickness_increase_fraction",
            "midcell_aft_shift_mm",
        ]
    })
    ...
    apply_fields("airfoil_processing", {
        "sharp_te_intersection_max_x_c": "sharp_te_intersection_max_x_c",
        "sharp_te_min_extension_x_c": "sharp_te_min_extension_x_c",
        "sharp_te_safe_gap_chord": "sharp_te_safe_gap_chord",
        "min_spline_point_distance_mm": "min_spline_point_distance_mm",
    })
    if values["te_closure_mode"] == "sharp_extension":
        values["create_te_closure_panels"] = False

    apply_fields("crossports", {
        "enable_crossports": "enable_crossports",
        "cut_mode": "crossport_cut_mode",
        "split_orientation": "crossport_split_orientation",
        "cut_strategy": "crossport_cut_strategy",
        "cutter_extrude_mode": "crossport_cutter_extrude_mode",
        "cutter_extrude_factor_semicell": "crossport_cutter_extrude_factor_semicell",
        "cutter_extrude_min_mm": "crossport_cutter_extrude_min_mm",
        "cutter_extrude_max_mm": "crossport_cutter_extrude_max_mm",
        "cutter_extrude_mm": "crossport_cutter_extrude_mm",
        "shape": "crossport_shape",
        "ellipse_orientation": "crossport_ellipse_orientation",
        "count": "crossport_count",
        "position_mode": "crossport_position_mode",
        "x_positions_chord": "crossport_x_positions_chord",
        "x_start_chord": "crossport_x_start_chord",
        "x_end_chord": "crossport_x_end_chord",
        "width_fraction_chord": "crossport_width_fraction_chord",
        "height_fraction_local_thickness": "crossport_height_fraction_local_thickness",
        "edge_clearance_fraction_local_thickness": "crossport_edge_clearance_fraction_local_thickness",
        "points_per_loop": "crossport_points_per_loop",
        "apply_to": "crossport_apply_to",
        "centerline_mode": "crossport_centerline_mode",
        "custom_specs": "crossport_custom_specs",
    })

    apply_fields("fabric_and_lines", {
        key: key for key in [
            "enable_fabric_thickness_properties",
            "fabric_thickness_mm",
            "fabric_density_kg_m3",
            "fabric_material",
            "fabric_thickness_strategy",
            "enable_fabric_thickness",
            "fabric_thickness_mode",
            "fabric_thickness_offset_side",
            "fabric_thickness_include_ribs",
            "fabric_thickness_include_stabilizers",
            "fabric_thickness_include_tip_bulge",
            "enable_suspension_tube_geometry",
            "default_suspension_line_diameter_mm",
    ...
    apply_fields("catia_exports", {
        "export_iges": "export_iges",
        "export_dxf": "export_dxf",
        "export_full_assembly_iges": "export_full_assembly_iges",
        "export_full_assembly_step": "export_full_assembly_step",
        "export_component_manifest": "export_component_manifest",
        "exports_subfolder": "export_subfolder_name",
        "export_canopy_filename": "export_canopy_filename",
        "export_full_filename": "export_full_filename",
    })
    catia_exports = data.get("catia_exports", {})
    if project_paths.get("catia_exports_dir") and "exports_subfolder" not in catia_exports:
        target = resolve_config_path(project_paths["catia_exports_dir"])
        values["export_subfolder_name"] = os.path.relpath(target, values["out_dir"]).replace("/", "\\")
    ...
    global CFD2D_FABRIC_THICKNESS_CHORD, CFD2D_MODEL_ZERO_THICKNESS_AS_THIN_SOLID
    ...
    if "fabric_thickness_chord" in airfoil:
        CFD2D_FABRIC_THICKNESS_CHORD = float(airfoil["fabric_thickness_chord"])
    if "model_zero_thickness_as_thin_solid" in airfoil:
        CFD2D_MODEL_ZERO_THICKNESS_AS_THIN_SOLID = bool(airfoil["model_zero_thickness_as_thin_solid"])

    global ENABLE_CANOPY_STABILIZERS, ENABLE_SUSPENSION_LINES, ENABLE_TIP_SIDE_BULGE, SYSTEM_CONFIG_JSON
    optional = data.get("optional_modules", {})
    if "enable_canopy_stabilizers" in optional:
        ENABLE_CANOPY_STABILIZERS = bool(optional["enable_canopy_stabilizers"])
    if "enable_suspension_lines" in optional:
        ENABLE_SUSPENSION_LINES = bool(optional["enable_suspension_lines"])
    if "enable_tip_side_bulge" in optional:
        ENABLE_TIP_SIDE_BULGE = bool(optional["enable_tip_side_bulge"])
    if optional.get("system_config_json"):
        SYSTEM_CONFIG_JSON = str(resolve_config_path(optional["system_config_json"]))

    global ENABLE_2D_CAE_EXPORTS, EXPORT_2D_OPEN_RAM_AIR_PROFILE, EXPORT_2D_CLOSED_REFERENCE_PROFILE
    global EXPORT_2D_PROFILE_PREVIEWS, EXPORT_2D_PROFILE_QUALITY_REPORT, EXPORT_2D_MESH_CONFIG_TEMPLATE
    cfd_exports = data.get("cfd_2d_exports", {})
    if "enable_2d_cae_exports" in cfd_exports:
        ENABLE_2D_CAE_EXPORTS = bool(cfd_exports["enable_2d_cae_exports"])
    if "export_open_ramair_profile" in cfd_exports:
        EXPORT_2D_OPEN_RAM_AIR_PROFILE = bool(cfd_exports["export_open_ramair_profile"])
    if "export_closed_reference_profile" in cfd_exports:
        EXPORT_2D_CLOSED_REFERENCE_PROFILE = bool(cfd_exports["export_closed_reference_profile"])
    if "export_profile_quality_report" in cfd_exports:
        EXPORT_2D_PROFILE_QUALITY_REPORT = bool(cfd_exports["export_profile_quality_report"])
    if "export_mesh_config_template" in cfd_exports:
        EXPORT_2D_MESH_CONFIG_TEMPLATE = bool(cfd_exports["export_mesh_config_template"])

    global WRITE_CHORD_DISTRIBUTION_PLOT
    debug_plots = data.get("debug_plots", {})
    if "write_chord_distribution_plot" in debug_plots:
        WRITE_CHORD_DISTRIBUTION_PLOT = bool(debug_plots["write_chord_distribution_plot"])
    if "write_2d_profile_previews" in debug_plots:
...
def default_system_config(cells: int = NUM_CELLS) -> dict:
    ...
    return {
        ...
        "constraints": {
            "R_over_b": 0.80,  #R line Length fixed by this factor 
...
def _load_system_json(out_dir: Path, cells: int) -> dict:
    import json
    configured = Path(SYSTEM_CONFIG_JSON)
    if configured.is_absolute():
        cfg_path = configured
    elif configured.parent != Path("."):
        cfg_path = PROJECT_ROOT / configured
    else:
        cfg_path = Path(out_dir) / configured
    if not cfg_path.exists():
        cfg_path.parent.mkdir(parents=True, exist_ok=True)
        cfg_path.write_text(json.dumps(default_system_config(cells), indent=2), encoding="utf-8")
    loaded = json.loads(cfg_path.read_text(encoding="utf-8"))
    if not isinstance(loaded, dict):
        raise TypeError(f"CATIA system configuration must contain a JSON object: {cfg_path}")
    # Keep the exact configuration used beside the CATIA inputs for provenance,
    # without making that generated snapshot the editable source of truth.
    snapshot = Path(out_dir) / "ramair_suspension_config.json"
    if snapshot.resolve() != cfg_path.resolve():
        snapshot.parent.mkdir(parents=True, exist_ok=True)
        snapshot.write_text(json.dumps(loaded, indent=2) + "\n", encoding="utf-8")
    return loaded
...
def write_2d_cae_exports(cfg: Config) -> None:
    ...
    if EXPORT_2D_MESH_CONFIG_TEMPLATE:
        ...
        mesh_cfg = {
            ...
            "closed_airfoil_target_nodes": 900,
            "closed_airfoil_transfinite_progression": 1.0,
            "closed_te_target_nodes": 25,
            "closed_te_transition_min_nodes": 24,
            "closed_te_neighbor_bump_enabled": True,
            "closed_te_neighbor_bump": 0.80,
            "closed_te_cap_distribution": "uniform",
            "closed_te_cap_progression": 1.0,
            "closed_single_curve_experimental": False,
            "closed_single_curve_kind": "BSpline",
            "closed_single_curve_target_nodes": 900,
            "closed_single_curve_start_at_te": True,
            "closed_single_curve_distribution": "bump",
            "closed_single_curve_bump": 0.06,
            "surface_size_from_boundary_layer_enabled": False,
            "surface_size_bl_outer_factor": 0.70,
            "surface_size_bl_outer_min_chord": 0.002,
            "surface_size_bl_outer_max_chord": 0.012,
            ...
            "debug_te_refinement_width_chord": 0.004,
            ...
            "nearfield_dist_min_chord": 0.02,
            "nearfield_intermediate_dist_chord": 0.45,
            "nearfield_dist_max_chord": 4.0,
            ...
            "open_lip_transfinite_min_nodes": 80,
            "open_inner_wall_node_factor": 0.55,
            "open_inner_te_node_factor": 0.10,
            "open_inner_wall_min_nodes": 80,
            "open_inner_te_min_nodes": 10,
            ...
            "open_inlet_transition_growth": 1.20,
            ...
            "open_surface_size_general_chord": 0.010,
            "open_surface_size_le_chord": 0.003,
            "open_surface_size_lip_chord": 0.0015,
            "open_surface_size_te_chord": 0.0012,
            "open_farfield_size_chord": 0.45,
            "open_cavity_wall_size_chord": 0.010,
            ...
            "open_surface_size_from_boundary_layer_enabled": False,
            "open_surface_size_bl_outer_factor": 0.70,
            "open_surface_size_bl_outer_min_chord": 0.002,
            "open_surface_size_bl_outer_max_chord": 0.014,
            ...
            "open_nearfield_dist_min_chord": 0.02,
            "open_nearfield_intermediate_dist_chord": 0.42,
            "open_nearfield_dist_max_chord": 2.80,
        ...
        mesh_cfg.update({
            ...
            "open_near_wall_size_chord": 0.004,
            ...
            "open_te_refinement_width_chord": 0.012,
            "open_te_transition_distance_chord": 0.010,
            "open_lip_transfinite_min_nodes": 56,
        ...
        # Keep new-project defaults identical to the canonical mesh builder.
        # The local literal above remains as a compatibility fallback for
        # standalone CAD-only packages that deliberately omit CFD scripts.
        mesh_scripts_dir = Path(__file__).resolve().parent / "CFD_2D" / "scripts"
        inserted_mesh_scripts = False
        if mesh_scripts_dir.is_dir() and str(mesh_scripts_dir) not in sys.path:
            sys.path.insert(0, str(mesh_scripts_dir))
            inserted_mesh_scripts = True
        try:
            from ramair_2d_mesh_builder import DEFAULT_MESH_CONFIG as canonical_mesh_defaults
        except ImportError:
            canonical_mesh_defaults = None
        finally:
            if inserted_mesh_scripts:
                sys.path.remove(str(mesh_scripts_dir))
        if canonical_mesh_defaults is not None:
            fabric_thickness_chord = mesh_cfg["fabric_thickness_chord"]
            mesh_cfg = dict(canonical_mesh_defaults)
            mesh_cfg["fabric_thickness_chord"] = fabric_thickness_chord
        mesh_cfg_path = root / "config" / "cfd2d_mesh_config.json"
        existing_mesh_cfg = {}
        if mesh_cfg_path.exists():
            try:
                existing_mesh_cfg = json.loads(mesh_cfg_path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError) as exc:
                raise RuntimeError(f"Cannot preserve existing editable mesh config {mesh_cfg_path}: {exc}") from exc
            if not isinstance(existing_mesh_cfg, dict):
                raise RuntimeError(f"Editable mesh config must contain a JSON object: {mesh_cfg_path}")
            # The preprocessor owns the template, not the user's tuning. Add new
            # keys without silently resetting an existing mesh configuration.
            mesh_cfg.update(existing_mesh_cfg)
            if "closed_boundary_layer_layers" not in existing_mesh_cfg and "boundary_layer_layers" in existing_mesh_cfg:
                mesh_cfg["closed_boundary_layer_layers"] = existing_mesh_cfg["boundary_layer_layers"]
            if "closed_boundary_layer_growth" not in existing_mesh_cfg and "boundary_layer_growth" in existing_mesh_cfg:
                mesh_cfg["closed_boundary_layer_growth"] = existing_mesh_cfg["boundary_layer_growth"]
            if "closed_first_cell_height_m" not in existing_mesh_cfg:
                legacy_y1c = existing_mesh_cfg.get(
                    "closed_first_cell_height_chord",
                    existing_mesh_cfg.get("first_cell_height_chord_override"),
                )
                if legacy_y1c is not None:
                    mesh_cfg["closed_first_cell_height_m"] = float(legacy_y1c) * chord_m
                    mesh_cfg["closed_use_yplus_first_cell_height"] = False
            if "open_first_cell_height_m" not in existing_mesh_cfg:
                legacy_open_y1c = existing_mesh_cfg.get(
                    "open_first_cell_height_chord",
                    existing_mesh_cfg.get("open_first_cell_height_chord_override"),
                )
                if legacy_open_y1c is not None:
                    mesh_cfg["open_first_cell_height_m"] = float(legacy_open_y1c) * chord_m
                    mesh_cfg["open_use_yplus_first_cell_height"] = False
            mesh_cfg["config_schema_version"] = 3
        compact_mesh_keys = [
            "config_schema_version", "geometry_mode", "fabric_thickness_chord",
            "gmsh_backend", "gmsh_threads", "gmsh_mesh_algorithm_2d", "gmsh_random_factor",
            "gmsh_random_seed",
            "run_boundary_layer", "target_y_plus", "extrude_to_3d_for_openfoam",
            "spanwise_thickness_chord", "spanwise_layers", "max_cells",
            "max_internal_parse_mesh_size_mb", "max_internal_parse_elements",
            "min_cells_warning", "stop_if_negative_cells", "stop_if_geometry_invalid",
            "closed_wall_curve_method", "closed_wall_target_nodes", "closed_te_bump_strength",
            "closed_te_target_nodes", "closed_te_transition_min_nodes",
            "closed_use_yplus_first_cell_height", "closed_first_cell_height_m",
            "closed_boundary_layer_layers", "closed_boundary_layer_growth",
            "closed_boundary_layer_total_thickness_chord", "closed_recombine_boundary_layer",
            "closed_boundary_layer_aniso_max_deg", "closed_boundary_layer_intersect_metrics",
            "closed_profile_preprocess_enabled", "closed_profile_target_points",
            "closed_profile_min_spacing_chord", "closed_te_rounding_enabled",
            "closed_te_rounding_points", "closed_te_rounding_window_chord",
            "closed_te_rounding_min_gap_chord", "closed_te_refinement_width_chord",
            "closed_te_refinement_strength", "closed_te_refinement_max_weight",
            "closed_near_wall_size_from_bl", "closed_near_wall_size_chord",
            "closed_near_wall_size_bl_factor", "closed_nearfield_enabled",
            "closed_nearfield_dist_min_chord", "closed_nearfield_intermediate_dist_chord",
            "closed_nearfield_dist_max_chord", "closed_nearfield_intermediate_size_chord",
            "closed_farfield_size_chord", "domain_radius_chord",
            "open_wall_curve_method", "open_single_connected_surface_2d",
            "open_inlet_boundary_layer_mode", "open_inlet_transition_elements",
            "open_inlet_transition_growth", "open_inlet_connector_normal_nodes",
            "open_use_yplus_first_cell_height",
            "open_first_cell_height_m", "open_boundary_layer_layers",
            "open_boundary_layer_growth", "open_boundary_layer_total_thickness_chord",
            "open_recombine_boundary_layer", "open_boundary_layer_aniso_max_deg",
            "open_boundary_layer_lip_fan_points",
            "open_minimum_fabric_thickness_chord", "open_near_wall_size_from_bl",
            "open_near_wall_size_chord", "open_near_wall_size_bl_factor",
            "open_surface_target_nodes", "open_surface_transfinite_multiplier",
            "open_surface_transfinite_progression", "open_te_transfinite_min_nodes",
            "open_lip_transfinite_min_nodes", "open_inner_wall_node_factor",
            "open_inner_te_node_factor", "open_inner_wall_min_nodes",
            "open_inner_te_min_nodes", "open_surface_size_le_chord",
            "open_surface_size_lip_chord", "open_surface_size_te_chord",
            "open_farfield_size_chord", "open_cavity_wall_size_chord",
            "open_cavity_wall_transition_chord", "open_cavity_size_chord",
            "open_internal_inlet_refinement_enabled", "open_internal_inlet_dist_min_chord",
            "open_internal_inlet_dist_max_chord", "open_internal_inlet_size_chord",
            "open_inlet_refinement_bridge_enabled", "open_inlet_marker_transfinite_nodes",
            "open_inlet_marker_bump_strength",
            "open_nearfield_refinement_enabled", "open_nearfield_dist_min_chord",
            "open_nearfield_intermediate_dist_chord", "open_nearfield_dist_max_chord",
            "open_nearfield_intermediate_size_chord", "wake_refinement_enabled",
        ]
        mesh_cfg = {key: mesh_cfg[key] for key in compact_mesh_keys if key in mesh_cfg}
        for key, value in existing_mesh_cfg.items():
            if key not in mesh_cfg and key not in legacy_template_keys:
                mesh_cfg[key] = value
        _json_write_2d(mesh_cfg_path, mesh_cfg)
        solver_cfg_path = root / "config" / "cfd2d_solver_config.json"
        solver_cfg = {
            "config_schema_version": 2,
            "solver": "foamRun",
        ...
        if solver_cfg_path.is_file():
            existing_solver_cfg = json.loads(solver_cfg_path.read_text(encoding="utf-8"))
            if not isinstance(existing_solver_cfg, dict):
                raise RuntimeError(f"Editable solver config must contain a JSON object: {solver_cfg_path}")
            is_unedited_legacy_template = (
                "config_schema_version" not in existing_solver_cfg
                and existing_solver_cfg.get("field_write_control") == "timeStep"
                and existing_solver_cfg.get("field_write_interval_steps") == 200
                and existing_solver_cfg.get("writeInterval_star") == 5
                and existing_solver_cfg.get("purgeWrite") == 5
            )
            if is_unedited_legacy_template:
                for legacy_key in ("writeInterval_star", "field_write_control", "purgeWrite"):
                    existing_solver_cfg.pop(legacy_key, None)
            solver_cfg.update(existing_solver_cfg)
            solver_cfg["config_schema_version"] = 2
        _json_write_2d(solver_cfg_path, solver_cfg)
...
def record_last_successful_preprocessor_config(args: argparse.Namespace) -> Path | None:
    """Persist the exact editable JSON only after a complete successful run."""
    raw_config = getattr(args, "config", None)
    if raw_config is None:
        return None
    source = _resolve_project_path(raw_config).resolve()
    if not source.is_file():
        return None
    destination = source.parent / "last_preprocessor_run_config.json"
...
def main() -> None:
    ...
    if WRITE_CHORD_DISTRIBUTION_PLOT:
    ...
    saved_config = record_last_successful_preprocessor_config(args)
    print(f"Generated CATIA input files in: {Path(cfg.out_dir).resolve()}")
    if saved_config is not None:
        print(f"Saved successful preprocessor configuration: {saved_config}")
